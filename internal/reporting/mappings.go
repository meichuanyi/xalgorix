...
// Mappings holds inferred security framework references for a single
// vulnerability — CWE id and name, OWASP Top 10 (2021) category id and
// name, and PTES phase. Empty strings indicate "unknown / no mapping".
...
// vulnTypeToCWE maps inferVulnType() output → CWE.
...
// vulnTypeKeywords mirrors the legacy report-pipeline classifier so the
// reporting package can infer mappings from a free-form title and
// description without reaching back into the web layer.
...
// InferMappings derives CWE, OWASP, and PTES mappings for a vulnerability.
// If the Vuln already has CWE/OWASP populated (agent-provided), those
// values take priority. Otherwise, keyword-based inference is used.
func InferMappings(v Vuln) Mappings {
	var m Mappings
